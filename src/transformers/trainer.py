...
class Trainer:
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        # If step tracking is enabled, wrap the DataLoader
        if self.args.step_tracking:
            total_steps = len(dataloader)

            # Wrap the DataLoader to include step_id and total_steps
            class StepTrackingDataLoader(DataLoader):
                def __iter__(loader_self):
                    for step_id, batch in enumerate(super(StepTrackingDataLoader, loader_self).__iter__(), start=1):
                        # Call the collator with step_id and total_steps
                        import pdb; pdb.set_trace()
                        yield data_collator(batch, step_id=step_id, total_steps=total_steps)

            # Use StepTrackingDataLoader
            dataloader = StepTrackingDataLoader(train_dataset, **dataloader_params)
